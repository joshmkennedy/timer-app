...
const SUBSCRIBE_PROMPT_DELAY = 2000;
...
export const PushNotificationsProvider: FunctionComponent<PushNotificationsProviderProps> = ({ children }) => {
	...
	const subscribeToConvex = useMutation(api.notifications.subscribe);
	const unsubscribeFromConvex = useMutation(api.notifications.unsubscribe);
	 ...
	 */
	useEffect(() => {
		if (deviceId && subscription && checkSubscriptionQuery !== undefined) {
			if (!checkSubscriptionQuery?.success) {
				subscription?.unsubscribe();
				setSubscription(null);
			}
		}
	}, [deviceId, subscription, checkSubscriptionQuery]);

	/**
	 * Determine if the user should be prompted to subscribe
	 */
	useEffect(() => {
		if (!subscription && subscriptionLoaded && deviceId && isAuthenticated) {
	...
	}, [subscription, subscriptionLoaded, deviceId, isAuthenticated]);
	...
	async function unsubscribeFromPush() {
		...
		try {
			...
			setSubscription(null);
			await unsubscribeFromConvex({ deviceId });
			setLoadingMessage(null);
		} catch (error) {
